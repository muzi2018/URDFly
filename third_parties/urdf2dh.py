...
class GenerateDhParams():
    ...
    def calculate_dh_params(self) -> None:
        ...
        for urdf_node in LevelOrderIter(self.root_node):
            if urdf_node.type == 'link' and (not self.urdf_links[urdf_node.id]['dh_found']):
                ...
                robot_dh_params.append([theta, d, a, alpha]) 
        
        return robot_dh_params
    ...
    def _process_intersection_case(self, origin, axis) -> np.ndarray:
        dh_params = np.zeros(4)
        # dh_params[0] = 1
        dh_params[0]  = gh.lines_intersect(np.zeros(3), np.array([0, 0, 1]), origin, axis)[1][0].item()
    ...
    def get_dh_parameters(self):
        ...
        return self.calculate_dh_params()
