...
class FK_SYM:
    ...
    def calulate_global_pos(self):
        ...
        for i in range(1, self.num_joints):
            ...
            last_global_tf = self.global_tf_list[-1]
        self.global_pos = self.global_tf_list[-1] * Matrix(
            [symbols("x"), symbols("y"), symbols("z"), 1.0]
        )
        self.global_pos = self.global_pos[0:3]
        return_global_pos = lambdify(
            [self.qs, ["x", "y", "z"]], self.global_pos, "numpy"
        )
    ...
    def calculate_global_pos_rot(self):
        ...
        self.global_pos_rot = self.global_tf_list[-1]
        return_global_pos_rot = lambdify(
            self.qs,
            self.global_pos_rot,
            "numpy",
        )
